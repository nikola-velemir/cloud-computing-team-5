...
export class NewAlbumForm implements OnInit, OnDestroy {
  ...
  constructor(private notifier: NgxNotifierService, private creationService: ContentCreationService) {
  }
  ...
  ngOnInit(): void {
    ...
    if (nameControl)
      this.nameFailureSub = nameControl.statusChanges
        .pipe(filter(s => s === 'INVALID')).subscribe(() => {
          this.notifier.createToast("Name is required", 'danger', 3000);
        })

    if (yearControl)
      this.yearFailureSub = yearControl.statusChanges.pipe(filter(s => s === 'INVALID')).subscribe(
        () => {
          this.notifier.createToast("Year is required", 'danger', 3000);
        });
    if (yearControl && nameControl)
      this.successSub = this.newAlbumForm.statusChanges.pipe(filter(s => s === 'VALID')).subscribe(
        () =>
          this.creationService.setCreatedAlbum({name: nameControl.value, year: nameControl.value} as AlbumCreation)
      )
