...
export class GenreView {
  ...
  ngOnInit(): void {
    this.reviewService.getReview(this.genreId).subscribe(review => this.reviewType = review);
  }
  ...
  dislike() {
    ...
    this.reviewService.setReview(this.genreId, type).pipe(
      switchMap(() => this.reviewService.getReview(this.genreId, type))
